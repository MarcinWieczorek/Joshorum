...
/**
 * Obsługa craftingu minecraft
 * @package Parser
 * @subpackage Filters
 * @author wookieb & CTRL
 * @version 1.1
 */
class BbCodeFilterRecipe {
	/**
	...
	public function findUrlCallback($matches) {
		...
		$this->_findUrlCallbackBB = $oc.'recipe=\''.$matches[0].'\''.$cc.$this->shortUrl($matches[0], self::URL_LENGTH).$oc.'/recipe'.$cc;
		...
		return '<script>ShowRecipe("'.$matches[0].'","'.$matches[1].'",'.$this->tags['url']['close'].'>';
	}
